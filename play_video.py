...
import yaml
from sd_utils import read_results_dict, evaluate, write_stats
from workspace.instance_strategy import StrategyFactory
...
def main(args):
    ...
    results, bw, lt = strategy.run(args)
    ...
    write_stats(args.outfile, f"{args.video_name}", config, f1, stats, bw, number_of_frames, mode,lt)
...
if __name__ == "__main__":
    ...
    if args.estimate_banwidth and not args.high_images_path:
        print("streamduet needs location of high resolution images to calculate true bandwidth estimate")
        exit()
